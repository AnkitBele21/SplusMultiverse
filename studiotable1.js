...
function processMultipleRows(data) {
    // Processing rows 2 to 5 for tables T1 to T4
    const activeTables = data.slice(1, 5).filter(row => row.length >= 8);
...
function displayMatchDetails(rowData) {
    ...
    const tableNumber = rowData[0]; // Assuming the table number is in the first column
